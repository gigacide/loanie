...
function handleSuccess() {
    ...
    // n = "http://127.0.0.1:8000/api/application/usa/post";
    n = "http://127.0.0.1:8000/process/us";
    fetch(n, {method: "POST",headers: {"Content-Type": "application/json"}, body: t})
        ...
        .then(n => {
            ...
            var checkStatusID = n.check_status_id
            console.log('checkStatus ID::', checkStatusID)
            console.log('POST::', n)
            debugger
            var i = "http://127.0.0.1:8000/check-status/us/" + checkStatusID, t = setInterval(() => {
                var r;
                fetch(i, {method: "GET", headers: {"Content-Type": "application/json"}}).then(n => n.json()).then(n => {
                    r = n;

                    var u = r.check_status_id, f = r.PercentageComplete, e = r.check_status, i = r.RedirectURL;
                    console.log(u)
                    debugger
                    if (trackProgress(f), i) return console.warn("redirecting"), progressDiv.classList.add("hide_element"), countdownDiv.classList.remove("hide_element"), startCountDown(u, i), trackFacebookConversion(r),
                        trackGoogleConversion(r), trackBingConversion(u, r),
                        clearInterval(t), !1;
...
function processFormData() {
    ...
    const vid = urlParams.get('vid') ?? 'AFF_UP151';
    const oid = urlParams.get('oid')  ?? '2';
    ...
    return cookies && cookies.gclid && (n = appendQueryParams(n, "gclid", cookies.gclid)), i = {
        ...
        vid: document.getElementById("vid").value ?? uping.defaultAffiliate,
        oid:  document.getElementById("oid").value ?? '',
        transaction_id: document.getElementById("transaction_id").value ?? '',
        aff_sub: document.getElementById("aff_sub").value ?? '',
        aff_sub2: document.getElementById("aff_sub2").value ?? '',
        aff_sub3: document.getElementById("aff_sub3").value ?? '',
        aff_sub4: document.getElementById("aff_sub4").value ?? '',
        aff_sub5: document.getElementById("aff_sub5").value ?? '',
        // fb_pix: params.fbpix ?? '',
        istest: true,
        response_type: 'json',
        ...
        bank: {
            ...
            // bankRoutingNumber: bankSortCode(),
            bankRoutingNumber: document.getElementById("sortcode").value,
...
function validateZipCode() {
    var zipCode = document.getElementById("postcode").value.trim();
    var messages = document.querySelector(".messages");
    var pattern = /^\d{5}(?:[-\s]\d{4})?$/; // pattern to match USA zip code

    if (!pattern.test(zipCode)) {
        messages.innerHTML = "Please enter a valid USA zip code (e.g. 12345 or 12345-6789)";
...
function validateSSN(ssn) {
    ssn = ssn.trim().replace(/[-\/\\^$*+?.()|[\]{}]/g, ''); // remove any slashes and hyphens
    var messages = document.querySelector(".messages");
    ...
    if (!pattern.test(ssn)) {
        ...
        messages.innerHTML = "Please enter a valid Social Security Number (e.g. XXXXXXXXX)";
        messages.classList.add("error");
        return false;
    } else {
        console.log('SSN:', ssn);
...
function validateDrivingLicenseNumber(drivingLicenseNumber) {
    // Remove any slashes and hyphens from the input value
    drivingLicenseNumber = drivingLicenseNumber.trim().replace(/[\/\-]/g, '');

    // Regular expression for USA Driving License Number validation
    const regex = /^[A-Z]{2}\d{7}$/;

    // Test the input against the regular expression
    if (!regex.test(drivingLicenseNumber)) {
        // If it fails the validation, set the error message
        document.querySelector('.messages').innerHTML = "Please enter a valid USA Driving License Number (e.g. AB1234567)";
        return false;
    }

    // Otherwise, clear the error message and return true
    document.querySelector('.messages').innerHTML = "";
    return true;
}

function validateRoutingNumber() {
    var routingNumber = document.getElementById("sortcode").value;
    var messages = document.querySelector(".messages");
    var pattern = /^(\d{9})$/; // pattern to match USA bank routing number

    if (!pattern.test(routingNumber)) {
        messages.innerHTML = "Please enter a valid USA bank routing number (9 digits)";
        messages.classList.add("error");
        return false;
    } else {
        messages.innerHTML = "";
