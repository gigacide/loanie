...
function setQueryParams() {
    var n = pingyo.getUrlVars(), t = document.getElementById(n.term);
...
function addTotalUserIncome() {
    ...
    f.innerHTML = "Your Total Monthly Income: &pound;" + n.value
}
    ...
    fetch(n, {method: "POST", headers: {"Content-Type": "application/json"}, body: t}).then(n => n.json()).then(n => {
        ...
        var i = "https://leads.uping.uk" + n.StatusCheckUrl, t = setInterval(() => {
            ...
            fetch(i, {method: "GET", headers: {"Content-Type": "application/json"}}).then(n => n.json()).then(n => {
                r = n;
                var u = r.CorrelationId, f = r.PercentageComplete, e = r.Status, i = r.RedirectionUrl;
                ...
                if (e == "NoLenderMatchFound" || f == 100 && !i) {
    ...
    fetch(n, {method: "POST", headers: {"Content-Type": "application/json"}, body: t}).then(n => n.json()).then(n => {
        ...
        var i = "https://leads.uping.uk" + n.StatusCheckUrl, t = setInterval(() => {
            ...
            fetch(i, {method: "GET", headers: {"Content-Type": "application/json"}}).then(n => n.json()).then(n => {
                ...
                if (e == "NoLenderMatchFound" || u == 100 && !i) {
                    countdownDiv.classList.add("hide_element");
                    console.warn("no lender");
                    statusText.innerHTML = "Could not match you with a lender at this time. Please try again.";
                    clearInterval(t);
...
function createImpression() {
    var t = document.referrer, i = pingyo.getVisitCookieName(), r = JSON.parse(pingyo.readVisitCookie(i)), n;
    if (r == undefined) return pingyo.createVisitCookie(), console.log(pingyo.getVisitCookieName()), n = new XMLHttpRequest, n.open("POST", "https://leads.uping.uk/application/impression/" + pingyo.defaultAffiliate, !1), n.send(t), n.responseText
}
...
function processFormData() {
    var t = calculateIncome(), r = calculateBank(), u = calculateMoveIn(),
        ...
        o = document.getElementById("employmentindustry").value.length > 0 ? Number(document.getElementById("employmentindustry").value) : 12,
        s = dobToEpoch(), n = document.referrer, h = getIpAddress("https://www.uping.uk/geo/echo/ip"),
        c = getLoanProceedUse(), l = pingyo.defaultAffiliate, i;
    if (!l) throw"Affiliate Id not detected. Fallback to TENDO";
    return cookies && cookies.gclid && (n = appendQueryParams(n, "gclid", cookies.gclid)), i = {
        ...
        Application: {
            ...
            MobilePhoneNumber: document.getElementById("mobilephone").value,
            WorkPhoneNumber: document.getElementById("mobilephone").value,
            ...
            EmploymentStarted: t.employmentstarted,
            ...
            NextPayDate: t.nextpaydate,
            FollowingPayDate: t.followingpaydate,
            ...
            AddressCity: document.getElementById("citytown").value,
            AddressCountryCode: "GB",
            AddressCounty: document.getElementById("county").value,
            AddressMoveIn: u,
            AddressPostcode: document.getElementById("postcode").value,
            ...
            BankRoutingNumber: document.getElementById("sortcode").value.replace(/\D/g, ""),
...
function trackGoogleConversion(n) {
    ...
    var t = n.EstimatedCommission.Amount, i = String(n.EstimatedCommission.CurrencyCode) || "GBP";
    try {
        gtag("event", "conversion", {send_to: "AW-702456292/D1FHCIHM8K4BEOTD-s4C", value: t, currency: i})
    } catch (r) {
        console.error(r)
    }
}
...
}, form = document.querySelector("form#pingYoForm"), inputs = document.querySelectorAll("input, textarea, select");
...
var mobilePhoneInput = document.getElementById("mobilephone"), homePhoneInput = document.getElementById("homephone"),
    workphoneInput = document.getElementById("mobilephone"), postalCodeInput = document.getElementById("postcode"),
    dobInput = document.getElementById("dob"), sortCodeInput = document.getElementById("sortcode"),
...
sortCodeInput.addEventListener("input", function (n) {
    var t = validate(form, {Bank_sort_code: {sortCodeValidator: n.target.value}});
    return t ? showErrorsForInput(this, t[this.name]) : null
});
